...
export const useHouseholds = () => {
  ...
  const executeUpdateCallbacks = useCallback(
    async (action, data) => {
      ...
      handleErrorSilently(error, "Execute Update Callbacks");
      }
    },
    [updateCallbacks]
  );
  ...
  const createHousehold = async (householdData) => {
    ...
    try {
      ...
      handleErrorSilently(err, "Save Household");
  ...
  const updateHousehold = async (householdId, householdData, options = {}) => {
    ...
    try {
      ...
      handleErrorSilently(err, "Update Household");
  ...
  const bulkUpdateHouseholds = async (updates) => {
    ...
    try {
      ...
      handleErrorSilently(err, "Bulk Update Households");
      toast({
  ...
  useEffect(() => {
    fetchHouseholds();
  }, [page, perPage, searchTerm, filterPurok, filterHousingType]);
