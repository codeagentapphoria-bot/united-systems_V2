...
// =============================================================================
// BIMS MATCH STATUS HELPER
// citizen_resident_mapping is not in the Prisma schema, so we query it raw.
// Returns: 'CONFIRMED' | 'PENDING' | 'NEEDS_REVIEW' | 'REJECTED' | 'NOT_FOUND'
// =============================================================================

type BimsMatchStatus = 'CONFIRMED' | 'PENDING' | 'NEEDS_REVIEW' | 'REJECTED' | 'NOT_FOUND';

/** Look up the match status for a single citizen. */
async function getBimsMatchStatus(citizenId: string): Promise<BimsMatchStatus> {
  const rows = await prisma.$queryRaw<{ status: string }[]>`
    SELECT status FROM citizen_resident_mapping
    WHERE citizen_id = ${citizenId}
    LIMIT 1
  `;
  if (!rows.length) return 'NOT_FOUND';
  return rows[0].status as BimsMatchStatus;
}

/** Batch look-up for a list of citizen IDs. Returns a Map citizenId → status. */
async function getBimsMatchStatusBatch(citizenIds: string[]): Promise<Map<string, BimsMatchStatus>> {
  if (!citizenIds.length) return new Map();
  const rows = await prisma.$queryRaw<{ citizen_id: string; status: string }[]>`
    SELECT citizen_id, status FROM citizen_resident_mapping
    WHERE citizen_id = ANY(${citizenIds})
  `;
  const map = new Map<string, BimsMatchStatus>();
  for (const row of rows) map.set(row.citizen_id, row.status as BimsMatchStatus);
  return map;
}
...
export const getRegistrationRequests = async (filters: RegistrationRequestFilters) => {
  ...
  // Attach BIMS match status to each request (batch query — avoids N+1)
  const citizenIds = requests.map(r => r.citizenId).filter(Boolean);
  const matchMap  = await getBimsMatchStatusBatch(citizenIds);
  const requestsWithBims = requests.map(r => ({
    ...r,
    bimsMatchStatus: matchMap.get(r.citizenId) ?? 'NOT_FOUND' as BimsMatchStatus,
  }));

  return {
    requests: requestsWithBims,
    pagination: {
      page,
      limit,
      total,
...
export const getRegistrationRequestById = async (id: string) => {
  ...
  const bimsMatchStatus = await getBimsMatchStatus(request.citizenId);

  return { ...request, bimsMatchStatus };
};
