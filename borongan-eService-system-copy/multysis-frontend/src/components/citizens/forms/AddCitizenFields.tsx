...
interface AddCitizenFieldsProps {
  currentStep: number;
  onStepChange?: (step: number) => void;
  validateStepRef?: React.MutableRefObject<((step: number) => Promise<boolean>) | null>;
}

export const AddCitizenFields: React.FC<AddCitizenFieldsProps> = ({ currentStep, onStepChange: _onStepChange, validateStepRef }) => {
  const form = useFormContext<AddCitizenInput>();
  const [citizenPicturePreview, setCitizenPicturePreview] = useState<string | null>(null);
  const [proofOfResidencyPreview, setProofOfResidencyPreview] = useState<string | null>(null);
  const [proofOfIdentificationPreview, setProofOfIdentificationPreview] = useState<string | null>(null);
  ...
  const generateUsername = async () => {
    ...
    try {
      ...
      for (const pattern of usernamePatterns) {
        ...
        const isAvailable = await citizenService.checkUsernameAvailability(pattern);
        if (isAvailable) {
      ...
      for (const base of basePatterns) {
        ...
        for (let i = 1; i <= 999; i++) {
          ...
          const isAvailable = await citizenService.checkUsernameAvailability(username);
          if (isAvailable) {
  ...
  const handleCitizenPictureChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        setCitizenPicturePreview(result);
      };
  ...
  const handleProofOfResidencyChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      // Create preview
      const reader = new FileReader();
      reader.onload = (e) => {
        const result = e.target?.result as string;
        setProofOfResidencyPreview(result);
      };
      reader.readAsDataURL(file);
      // Store file object instead of base64
      form.setValue('proofOfResidencyFile', file);
    }
  };
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div>
        ...
        <h3 className={cn("text-lg font-semibold text-primary-600 border-b pb-2")}>
          Personal Information
        </h3>

        <div className={cn("grid grid-cols-1 md:grid-cols-12 gap-6")}>
                ...
                <div className="relative group w-48 h-48">
                  ...
                  >
                    ...
                    <input
                      ...
                      accept="image/*"
                      className="hidden"
                      onChange={handleCitizenPictureChange}
                    />
                  </label>
                </div>
        ...
        render={() => (
          <FormItem className="space-y-4">
            ...
            <p className="text-sm text-gray-600">
              Upload a document that proves the citizen's residency (e.g., utility bill, lease agreement, etc.)
            </p>
