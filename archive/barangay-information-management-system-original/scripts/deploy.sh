...
# Enhanced Production Deployment Script for BIMS
# This script builds and deploys the BIMS application for production
...
while [[ $# -gt 0 ]]; do
    case $1 in
        -v|--verbose)
            ...
            ;;
        -h|--help)
            echo "BIMS Production Deployment Script"
...
echo "🚀 Starting BIMS Production Deployment..."
...
echo "📝 Deployment logs will be saved to: $DEPLOYMENT_LOG"
if [ "$VERBOSE" = true ]; then
    echo "🔍 Verbose mode enabled - detailed logs will be displayed in real-time"
fi
...
log_message() {
    ...
    # Display to console if verbose mode is enabled
    if [ "$VERBOSE" = true ]; then
...
cleanup_disk_space() {
    ...
    # Note: Backup cleanup removed as backup functionality is not needed
...
# Note: backup_existing function removed as backup functionality is not needed
...
setup_backend() {
    ...
    # Copy production environment to .env for server compatibility (only if .env doesn't exist)
    if [ -f ".env.production" ] && [ ! -f ".env" ]; then
        ...
        print_warning "No .env.production file found - using existing .env if available"
    fi
    
    # Run database migrations (schema only, no seeding during deployment)
    print_status "Running database schema migrations..."
    NPM_PATH=$(get_npm_path)
    
    # Skip seeding steps: Step 5 (Data Seeding) and Step 7 (Classification Types Seeding)
    # Only run schema migrations during deployment
    if NODE_ENV=production "$NPM_PATH" run db:migrate --skip-step=5 --skip-step=7 2>&1 | grep -q "already exists"; then
        print_warning "Database tables already exist, skipping migration"
    elif NODE_ENV=production "$NPM_PATH" run db:migrate --skip-step=5 --skip-step=7; then
        print_success "Database schema migrations completed"
    else
        print_warning "Database migration failed, but continuing..."
    fi
    
    # Note: Database seeding is not performed during deployment
    # Seeding should only be done during initial setup, not deployment
    # Skipped steps: 5 (Data Seeding) and 7 (Classification Types Seeding)
...
setup_nginx() {
    ...
    # Get IP address
    local instance_ip=$(curl -s http://169.254.169.254/latest/meta-data/public-ipv4 2>/dev/null || hostname -I | awk '{print $1}')
    print_status "Using server name: $instance_ip"
...
server {
    listen 80;
    server_name $instance_ip localhost;
...
# Fix Mixed Content HTTPS/HTTP issues
fix_mixed_content_issues() {
    print_status "Fixing Mixed Content HTTPS/HTTP issues..."
    
    # Get the instance IP
    local instance_ip=$(curl -s http://169.254.169.254/latest/meta-data/public-ipv4 2>/dev/null || hostname -I | awk '{print $1}')
    
    # Fix client .env to use HTTPS endpoints
    if [ -f "$PROJECT_DIR/client/.env" ]; then
        print_status "Updating client .env to use HTTPS endpoints..."
        
        # Note: .env backup removed as backup functionality is not needed
        ...
        # Update specific IP addresses to use HTTPS
    ...
    # Fix server CORS to allow HTTPS origins
    if [ -f "$PROJECT_DIR/server/.env.production" ]; then
        print_status "Updating server CORS to allow HTTPS origins..."
...
fix_deployment_issues() {
    ...
    if [[ ! -f "$ssl_cert" ]] || [[ ! -f "$ssl_key" ]]; then
        ...
        # Get the IP address for the certificate
        local cert_cn=$(curl -s --max-time 5 http://169.254.169.254/latest/meta-data/public-ipv4 2>/dev/null || hostname -I | awk '{print $1}')
        
        print_status "Creating SSL certificate for: $cert_cn"
        
        # Create SSL certificate
        sudo openssl req -x509 -nodes -days 365 -newkey rsa:4096 \
            -keyout "$ssl_key" \
            -out "$ssl_cert" \
            -subj "/C=PH/ST=Eastern Samar/L=Borongan/O=BIMS/CN=$cert_cn" \
            -addext "subjectAltName=DNS:$cert_cn,DNS:localhost,IP:$cert_cn,IP:127.0.0.1" 2>/dev/null && {
...
### Camera not working:
1. Ensure using HTTPS: https://$(hostname -I | awk '{print $1}')
...
main() {
    echo "🏛️ BIMS Production Deployment"
    echo "=============================="
    ...
    fix_mixed_content_issues || ((deployment_errors++))
    ...
    echo "💡 To view logs in real-time during deployment, use: npm run deploy -- --verbose"
    fi
    ...
    echo "HTTPS Frontend: https://$instance_ip (Recommended - Camera access)"
    echo "HTTPS Backend API: https://$instance_ip/api"
    echo "HTTP Frontend: http://$instance_ip (Redirects to HTTPS)"
    echo ""
    echo "📱 Camera Functionality:"
    echo "• Camera access requires HTTPS - use https://$instance_ip"
