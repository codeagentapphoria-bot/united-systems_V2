...
const EditOfficialDialog = ({
  ...
  const { handleCRUDOperation } = useUnifiedAutoRefresh({
    ...
    successMessage: 'Official updated successfully!',
    errorMessage: 'Failed to update official',
    showToast: true,
    autoRefresh: true,
    refreshDelay: 100
  });
  ...
  // When editing, allow the current official to keep their position
  const availablePositionOptions = POSITION_OPTIONS.filter((option) => {
    const existingPosition = existingOfficials.find(
      (existingOfficial) =>
        existingOfficial.position === option.value &&
        existingOfficial.official_id !== official?.official_id
    );

    // For single-occupancy positions, check if they already exist (excluding current official)
    ...
    if (singleOccupancyPositions.includes(option.value) && existingPosition) {
      return false; // Don't show if position is already occupied by another official
    }
  ...
  // Fetch resident by ID for display in edit mode
  const fetchResidentById = async (residentId) => {
    try {
      const response = await api.get(`/${residentId}/resident`);
      const resident = response.data.data;
      if (resident) {
        setSelectedResident(resident);
        // Also add to residents array so it's available in options
        setResidents(prev => {
          const exists = prev.some(r => r.id === residentId);
          return exists ? prev : [...prev, resident];
        });
      }
    } catch (error) {
      // If API call fails, create a placeholder from the official data
      if (official) {
        const placeholderResident = {
          id: residentId,
          first_name: official.resident_first_name || 'Unknown',
          last_name: official.resident_last_name || 'Unknown',
          suffix: official.resident_suffix || '',
        };
        setSelectedResident(placeholderResident);
      }
    }
  };

  // Populate form with official data
  useEffect(() => {
    if (official && open) {
  ...
  console.error("Error formatting date:", error);
      ...
      setFormData({
        barangayId: official.barangay_id || barangayId,
        residentId: official.resident_id || "",
        position: official.position || "",
        committee: official.committee || "",
        termStart: formatDateForInput(official.term_start),
        termEnd: formatDateForInput(official.term_end),
        responsibilities: official.responsibilities || "",
      });

      // If we have a resident_id, fetch the resident data to display
      if (official.resident_id) {
        // Check if official already has resident data
        if (official.resident_first_name && official.resident_last_name) {
          const residentData = {
            id: official.resident_id,
            first_name: official.resident_first_name,
            last_name: official.resident_last_name,
            suffix: official.resident_suffix || '',
          };
          setSelectedResident(residentData);
        } else {
          // Fetch resident data from API
          fetchResidentById(official.resident_id);
        }
      }
    }
  }, [official, open, barangayId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await handleCRUDOperation(
        async (data) => api.put(`/${official.official_id}/official`, data),
        formData
      );
      onSuccess();
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the official's information. All fields marked with * are
            required. Note: Only one person per position for Punong Barangay, SK
